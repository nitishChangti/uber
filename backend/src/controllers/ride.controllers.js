...
const createNewRide = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .json(new ApiResponse(201, { ride }, "Ride created successfully", true));
...
const getFareRide = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, { fare }, "Fare calculated successfully", true)
      );
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: "Server error" });
...
const confirmRide = asyncHandler(async (req, res) => {
  ...
  const ride = await Ride.findOne({ _id: rideId })
    ...
    .populate("captain", "-password -refreshToken")
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { ride }, "Ride confirmed successfully", true));
});
...
const startRide = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { ride }, "Ride started successfully", true));
});
const finishRide = asyncHandler(async (req, res) => {
  ...
  const commissionRate = 0.1; // 10% commission (you can change)
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { ride }, "Ride finished successfully", true));
});
const getRecentRideNotifications = asyncHandler(async (req, res) => {
  console.log("Recent ride notification controller hit");

  const userId = req.user._id;

  const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000);

  // 🔹 Fetch ONLY latest ride
  const ride = await Ride.findOne({
    user: userId,
    createdAt: { $gte: tenMinutesAgo },
    status: { $in: ["pending", "accepted", "ongoing"] },
  })
    .select("+otp fare pickup destination status captain createdAt")
  ...
  if (ride.status === "pending") {
    type = "RIDE_PENDING";
    message = "Your ride request is still pending.";
  }

  if (ride.status === "accepted") {
    type = "RIDE_ACCEPTED";
    message = `Your ride has been accepted by ${ride.captain?.fullName}`;
  }

  if (ride.status === "ongoing") {
    type = "OTP_REQUIRED";
    message = "Your ride has started. Please share OTP with captain.";
  }
...
const cancelRideByCaptain = asyncHandler(async (req, res) => {
  console.log(`this is ride cancel controller for captain`);
  const captainId = req.captain._id; // from captain auth middleware
  const { rideId } = req.body;
  ...
  // Ensure this captain owns the ride
  ...
  // Prevent cancelling after ride started
  ...
  ride.cancelReason = "Captain cancelled the ride";

  await ride.save();

  // (Optional) notify user via socket
  req.io?.to(ride.user.toString()).emit("ride-cancelled", {
    rideId,
    reason: "Captain cancelled the ride",
  });
