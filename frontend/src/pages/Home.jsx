...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const getRecentRideNotification = async () => {
      try {
        const accessToken = JSON.parse(localStorage.getItem("accessToken"));
        console.log(`accessToken is ${accessToken}`);
        const response = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/rides/recent`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
            withCredentials: true, // ✅ must be inside SAME config object
          }
        );
        const notification = response?.data?.notification;
        if (!notification) return;

        console.log("Recovered notification:", notification);

        // 🔑 STATE RESTORATION (THIS WAS MISSING)
        if (notification.type === "RIDE_ACCEPTED") {
          setConfirmRideData(notification);
          setWaitingForDriver(true); // ✅ REQUIRED
          setVehicleFound(false);
          setIsRideRequested(false);
        }

        if (notification.type === "RIDE_PENDING") {
          setVehicleFound(true);
          setWaitingForDriver(false);
        }

        if (notification.type === "OTP_REQUIRED") {
          navigate("/riding", {
            state: { rideData: notification },
          });
        }
      } catch (error) {
        console.error(
          "something wrong went while getting a recent ride notification for user",
          error
        );
        console.log(
          "something wrong went while getting a recent ride notification for user"
  ...
  return (
    <div className=" relative w-full h-screen border-2">
      ...
      <div
        ref={containerRef}
        className={`absolute ${
          panelOpen ? " h-screen" : "bottom-0 "
        }  bg-white w-full flex-col border-0 flex rounded-t-2xl `}
      >
        <div className="p-5 w-full flex flex-col gap-5 h-[30%] ">
          <h1
            ref={panelCloseRef}
            onClick={(e) => setPanelOpen(false)}
            className="absolute right-10 text-3xl opacity-0"
          >
        ...
        >
          <LocationSearchPanel
            ...
            findTrip={findTrip}
            useCurrentLocation={useCurrentLocationAsPickup} // ⭐ NEW
          />
        </div>
      </div>
      <div
