...
const UserSignUp = () => {
    ...
    const onSubmit = async (data) => {
        console.log(data);
        try {
            const response = await authService.register(data.fullName, data.email, data.password, data.phoneNumber);
        ...
        <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-b from-white to-gray-100 px-4">
            ...
            <div className="mb-8 w-full max-w-sm">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    {/* Full Name Input */}
                    <label className="block mb-2 text-sm font-medium text-gray-700">What's your name</label>
                    <input
                    ...
                    />
                    {errors.fullName && <p className="text-red-500 text-xs mb-4">{errors.fullName.message}</p>}
                    ...
                    <input
                        ...
                        className="w-full px-4 py-2 mb-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black"
                    />
                    {errors.phoneNumber && <p className="text-red-500 text-xs mb-4">{errors.phoneNumber.message}</p>}
                    ...
                    <input
                        ...
                        className="w-full px-4 py-2 mb-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black"
                    />
                    {errors.password && <p className="text-red-500 text-xs mb-6">{errors.password.message}</p>}

                    {/* Sign Up Button */}
                    <button type="submit" className="w-full bg-black text-white py-3 rounded-md font-semibold text-base hover:opacity-90 mt-5">
                        Sign Up
                    </button>
                </form>
